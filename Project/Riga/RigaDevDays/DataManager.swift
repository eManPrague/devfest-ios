...
class DataManager {
    ...
    let customImageURLPrefix = ""//"https://2017.devfest.cz"
...
//            && resourcesReceived
    ...
    func startObservingPublicData() {
        ...
        rootRef.child("partners").observe(.value, with: { snapshot in
            self.partnerGroups = [PartnerGroup()] + snapshot.children.map{ PartnerGroup(snapshot: $0 as! DataSnapshot) }
        ...
        rootRef.child("maplayer").observe(.value, with: { snapshot in
            guard let kmlString = snapshot.value as? String else { return }
            guard let data = try? Data(contentsOf: URL(string: kmlString)!) else { return }
            
            let manager = FileManager.default
            let url = manager.urls(for: .documentDirectory, in: .userDomainMask).first!
            let filePath = url.appendingPathComponent("layer.kml")
            ...
            do {
                ...
                do {
                    ...
                    self.kmlURL = filePath
                    if self.allDataReceivedNotificationSent { NotificationCenter.default.post(name: .KMLUpdated, object: nil) }
                } catch { }
    ...
    func startMonitoringUser() {
        handle = Auth.auth().addStateDidChangeListener() { (auth, user) in
            if (user != nil) {
                ...
                GameDataManager.shared.userToBeSaved = user
                LoginManager.shared.afterLogin()
                NotificationCenter.default.post(name: .UserDidSignIn, object: nil)
            }
            else {
                LoginManager.shared.afterLogout()
