...
//        case Map
    ...
    @IBAction func twitter(_ sender: UIButton) {
        UIApplication.shared.openURL(URL(string: "https://twitter.com/DevFest_CZ")!)
    }
...
extension SessionViewController: UITableViewDataSource {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return 6 // -> amount of Section enum
    }
        ...
        case TableSections.AddToCalendar.rawValue:
            ...
            if SwissKnife.sharedInstance.calendarEvent(for: self.session!, on: properDay!) != nil {
                ...
                SwissKnife.sharedInstance.getEventDialogFor(self.session!, on: properDay!, completion: { [weak self] (controller) in
                    ...
                    let alertController = UIAlertController(title: locs("calendar.permission-denied"), message: locs("calendar.change-in-settings"), preferredStyle: .alert)
                    
                    let settingsAction = UIAlertAction(title: locs("calendar.go-to-settings"), style: .default, handler: { _ in
                        UIApplication.shared.openURL(URL(string: UIApplicationOpenSettingsURLString)!)
                    })
                    let cancelAction = UIAlertAction(title: locs("calendar.cancel"), style: .cancel, handler: nil)
